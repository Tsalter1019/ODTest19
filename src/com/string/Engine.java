package com.string;

import java.util.Scanner;

/**
 * 基因序列可以表示为一条由 8 个字符组成的字符串，其中每个字符都是 'A'、'C'、'G' 和 'T' 之一。
 *
 * 假设我们需要调查从基因序列start 变为 end 所发生的基因变化。一次基因变化就意味着这个基因序列中的一个字符发生了变化。
 *
 * 例如，"AACCGGTT" --> "AACCGGTA" 就是一次基因变化。
 * 另有一个基因库 bank 记录了所有有效的基因变化，只有基因库中的基因才是有效的基因序列。
 *
 * 给你两个基因序列 start 和 end ，以及一个基因库 bank ，请你找出并返回能够使start 变化为 end 所需的最少变化次数。
 * 如果无法完成此基因变化，返回 -1 。
 *
 * 注意：起始基因序列start 默认是有效的，但是它并不一定会出现在基因库中。
 *
 */
public class Engine {
    public int minMutation(String start, String end, String[] bank) {
        //记录需变异次数
        int times=0;
        for(int m=0;m<bank.length;m++){
            if(defineChange(start,bank[m])){
                times++;

                start = bank[m];
                if(start.equals(end)){
                    return times;
                }
            }
        }
        if(times>bank.length||times<=0){
            return -1;
        }else{
            return times;
        }

    }

    //判断是否发生了一次基因变化
    public Boolean defineChange(String strB,String strA){
        char [] chB = strB.toCharArray();
        char [] chA = strA.toCharArray();
        if(chB.length!=chA.length){
            return false;
        }else{
            int count = 0;
            for(int i=0;i<chB.length;i++){
                if(chB[i]==chA[i]){
                    count++;
                }
            }
            if(count==chA.length-1){
                return true;
            }
        }
        return false;
    }

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        String a = sc.nextLine();
        String b = sc.nextLine();
        String c = sc.nextLine();
        String [] d = c.split("\\ ");
        System.out.println(new Engine().minMutation(a,b,d));
    }
}