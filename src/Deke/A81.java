...
/**
 ...
 任何数字位置遇到数字4就直接跳过，其余功能都正常。比如：1. 23再多一块钱就变为25； 2. 39再多一块钱变为50；3. 399再多一块钱变为500；
...
public class A81 {
    public static void main(String[] args) {
        System.out.println(solution(322));//263
        System.out.println(solution(653));//444
        System.out.println(solution(500));//324
        System.out.println(solution(100));//81
        System.out.println(solution(17));//15
        System.out.println(solution(5));//4
    }
    public static int solution(int n){
        int sum=0;//包含重复计算
        for(int i=1;i<=n;i++) {
            if(returnNumberOfFour(i)>0){
                sum+=returnNumberOfFour(i);
            }
        }
        //计算重复计算的4
        return n-sum;
    }
    public static int returnNumberOfFour(int a){
